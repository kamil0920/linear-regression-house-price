...
class FeatureSelector(BaseEstimator, TransformerMixin):
    ...
    def get_methods(self, method_order):
        ...
        for method_label in method_order:
            ...
            if method_label == 'Constant Features':
                ordered_methods.update({method_label: self.constant_features})

            elif method_label == 'Correlated Features':
                ordered_methods.update({method_label: self.correlated_features})

            elif method_label == 'Relevant Features':
                ordered_methods.update({method_label: self.relevant_features})

            elif method_label == 'RFECV Features':
                ordered_methods.update({method_label: self.rfecv_features})

        return ordered_methods

    def constant_features(self, X, frac_constant_values=0.90):
        """
    ...
    def correlated_features(self, X, correlation_threshold=0.90):
        """
    ...
    def relevant_features(self, X, y, params):
        """
        ...
        gscv = GridSearchCV(estimator, param_grid, cv=5, n_jobs=-1, verbose=1, scoring='neg_mean_squared_error')
        ...
        # y = y.iloc[X.index.to_list()]

        # Initiate progress bar
        status.printProgressBar(counter, 5, prefix='Progress:', suffix='Complete', length=50)
    ...
    def rfecv_features(self, X, y, rfecv_params):
        """
